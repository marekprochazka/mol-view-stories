...
import { positionFromParam, positionToParam } from './types';
import type { PrimitiveKindFieldsProps } from './types';

export function EllipseFields({ params, onUpdate }: PrimitiveKindFieldsProps) {
  ...
  return (
    <div className='space-y-3'>
      ...
      <div className='grid grid-cols-2 gap-3'>
        <div>
          <Label className='text-xs font-medium'>Major Radius</Label>
          <Input
            ...
            min='0'
            placeholder='auto'
            value={typeof params.major_radius === 'number' ? params.major_radius : ''}
            onChange={(e) => {
              const v = e.target.value === '' ? undefined : parseFloat(e.target.value);
              const next = { ...params };
              if (v === undefined) delete next.major_radius;
              else next.major_radius = v;
              onUpdate(next);
            }}
          />
        </div>
        <div>
          <Label className='text-xs font-medium'>Minor Radius</Label>
          <Input
            className='h-8 text-sm font-mono mt-1'
            type='number'
            step='0.1'
            min='0'
            placeholder='auto'
            value={typeof params.minor_radius === 'number' ? params.minor_radius : ''}
            onChange={(e) => {
              const v = e.target.value === '' ? undefined : parseFloat(e.target.value);
              const next = { ...params };
              if (v === undefined) delete next.minor_radius;
              else next.minor_radius = v;
              onUpdate(next);
            }}
          />
        </div>
