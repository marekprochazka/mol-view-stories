...
export function LinesFields({ params, onUpdate }: PrimitiveKindFieldsProps) {
  return (
    <div className='space-y-3'>
      <p className='text-xs text-muted-foreground border rounded-md p-3 bg-muted/40'>
        Lines requires vertex/index data. Use the <strong>Raw</strong> tab to supply full params.
      </p>
      ...
      <div className='grid grid-cols-2 gap-3'>
        ...
        <div>
          ...
          <Input
            ...
            min='0'
            placeholder='auto'
            value={typeof params.line_width === 'number' ? params.line_width : ''}
            onChange={(e) => {
              const v = e.target.value === '' ? undefined : parseFloat(e.target.value);
              const next = { ...params };
              if (v === undefined) delete next.line_width;
              else next.line_width = v;
              onUpdate(next);
            }}
          />
        </div>
